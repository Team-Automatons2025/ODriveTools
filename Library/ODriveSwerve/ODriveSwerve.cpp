...
void ODriveSwerve::goTo(float position, float velocity){
    ...
    setParameter("axis0.trap_traj.config.accel_limit", 30);
    setParameter("axis0.trap_traj.config.decel_limit", 30);
...
ODriveProcedureResult ODriveSwerve::getProcedureResult() {
	return getParameterAsInt("axis0.procedure_result");
}

ODriveError ODriveSwerve::getError() {
	return getParameterAsInt("axis0.error");
}
